import React, { useContext } from 'react';
import { AppContext } from '../../../App';

const AdGemOffersPage: React.FC = () => {
    const { setCurrentPage } = useContext(AppContext);

    return (
        <div className="space-y-8">
            <div className="flex items-center gap-4">
                 <button onClick={() => setCurrentPage('Offer')} className="cyber-button-primary !py-2 !px-3">
                    <i className="fas fa-arrow-left"></i>
                </button>
                <div>
                    <h1 className="text-3xl font-bold text-cyber-primary text-glow-primary mb-2">AdGem</h1>
                    <p className="text-cyber-text-secondary">Complete offers from AdGem to earn rewards.</p>
                </div>
            </div>

            <div className="cyber-panel p-6 text-center">
                <p className="text-cyber-text-secondary">
                    API integration for AdGem would be placed here. This area would display available offers dynamically.
                </p>
                <div className="mt-8 w-full h-96 bg-cyber-surface flex items-center justify-center rounded-lg border-2 border-dashed border-cyber-primary/30">
                    <p className="text-cyber-text-secondary">Offer Wall Iframe Placeholder</p>
                </div>
            </div>
        </div>
    );
};

export default AdGemOffersPage;